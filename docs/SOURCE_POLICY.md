...
| LibriVox | `https://librivox.org/api/feed/audiobooks` using the official audiobook API | Listen, RSS audio feed, and MP3 ZIP when the record supplies it | LibriVox states that its texts and recordings are public domain in the US. US context is source-verified; UK and global contexts remain source-jurisdiction-only with a local-law warning. |

The Wikisource, DOAB, Library of Congress, and LibriVox integrations require no API key. They use documented JSON endpoints, an identifying user agent, bounded page sizes and timeouts, CDN caching, and independent failure states. LibreLeaf does not scrape HTML pages.
...
Every catalogue remains retryable when it times out; its cursor is not advanced or marked exhausted. All six sources start concurrently and share a 2.5-second first-results deadline. Each adapter's narrower timeout still applies, but no adapter may extend the overall search beyond the shared upstream budget. Open Library and LibriVox are attempted once per request; an unchanged cursor makes the next page request the retry instead of delaying healthy sources with an inline retry.
