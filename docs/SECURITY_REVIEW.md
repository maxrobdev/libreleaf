...
- LibreSend Local binds its control UI to loopback only behind a random path. It writes one bounded EPUB/PDF/MOBI to a mode-0600 temporary file, exposes only that file through a separate random expiring LAN path, enables no CORS or uploads on the receiving server, and removes the temporary file on replacement, removal, expiry or shutdown. Its local HTTP receiver is explicitly limited to trusted networks.
