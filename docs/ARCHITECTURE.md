...
This is not yet the production primary read path. The bounded snapshot builder can exhaust resolver cursors for a versioned query corpus, merge repeated canonical works and report incomplete/stuck crawls without claiming completion. Source-specific ingestion now includes Project Gutenberg's weekly CSV and DOAB's OAI-PMH catalogue, with archived inputs, explicit completeness and separate metadata/book-licence claims. Live source adapters remain active while the remaining full-catalogue imports, freshness and source-specific tombstone policies are completed. The index is designed to become the primary catalogue layer without removing live refresh and availability checks.
